...
az network vnet subnet update --name ${SUBNET_PVT_NAME} --resource-group ${VNET_RG_NAME} --vnet-name ${VNET_NAME} --disable-private-endpoint-network-policies true
...
az network private-endpoint create --name pvt-${PREFIX}Endpoint --resource-group ${VNET_RG_NAME} --location ${LOCATION} --vnet-name ${VNET_NAME} --subnet ${SUBNET_PVT_NAME} --connection-name conn-${PREFIX} --private-connection-resource-id ${RESOURCE_ID} --group-id sites

# Create a private DNS zone to resolve the name of the app service
# az network private-dns zone create --name ${PREFIX}privatelink.azurewebsites.net --resource-group ${VNET_RG_NAME}
...
VNET_RESOURCE_ID=$(az network vnet show --name ${VNET_NAME} --resource-group ${VNET_RG_NAME} --query "id" | tr -d '"')
az webapp vnet-integration add -g ${RG_NAME} -n ${APP_SVC_NAME} --vnet ${VNET_RESOURCE_ID} --subnet ${SUBNET_INT_NAME}