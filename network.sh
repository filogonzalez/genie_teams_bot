...
az network firewall update --name ${FIREWALL_NAME} --resource-group ${VNET_RG_NAME} --enable-dns-proxy true
az network vnet update --name ${VNET_NAME} --resource-group ${VNET_RG_NAME} --dns-servers ${FW_INT_ADD}

# Create a public IP for the firewall
az network public-ip create --name ${FIREWALL_NAME}-pip --resource-group ${VNET_RG_NAME} --location ${LOCATION} --allocation-method static --sku standard

# Associate the IP with the firewall
az network firewall ip-config create --firewall-name ${FIREWALL_NAME} --name ${FIREWALL_NAME}-Config --public-ip-address ${FIREWALL_NAME}-pip --resource-group ${VNET_RG_NAME} --vnet-name ${VNET_NAME}

# Update the firewall
az network firewall update --name ${FIREWALL_NAME} --resource-group ${VNET_RG_NAME}